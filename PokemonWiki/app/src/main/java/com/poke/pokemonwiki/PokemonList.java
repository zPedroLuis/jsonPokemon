...
public class PokemonList {
    ...
    public List<String> returnPokemonByName(String band){
        List<String> found = new LinkedList<String>();
        for(Pokemon pokemon: pokemons){
            if(pokemon.getPokemonName().equals(band)) found.add(pokemon.getType1());
        }
        return found;
    }
    ...
    public String returnRegion (String region){
        for (Pokemon pokemon: pokemons){
            if(pokemon.getPokemonName().equals(region)) return pokemon.getRegion();
        }
        return "Sorry...";
    }
